...
if "%errorlevel%"=="11" (
    echo.
    echo [ERROR] Python is installed but missing required standard modules ^(venv/tkinter^).
    echo.
...
set "PYTHON_STD_OK="
for /f %%I in ('%PYTHON_CMD% -c "import importlib.util; required=('venv','tkinter'); print(1 if all(importlib.util.find_spec(name) for name in required) else 0)" 2^>nul') do set "PYTHON_STD_OK=%%I"
